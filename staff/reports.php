  ...
  <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
    ...
    <div class="d-flex flex-wrap flex-md-nowrap align-items-center gap-2.5 mb-3">
      ...
      <div class="policy-search-box flex-grow-1">
        ...
        <input type="text" id="recentStaffReportsSearchInput" class="policy-search-input"
          placeholder="Search generated reports by file name, policy subject, or date..."
          aria-label="Search generated reports" oninput="onRecentStaffReportsSearchInput()">
        <button type="button" class="policy-search-clear" id="recentStaffReportsClearSearchBtn" title="Clear search"
          style="display: none;" onclick="clearRecentStaffReportsSearch()">
...
<script>
  ...
  function buildSharedReportTemplate(repObj, logoUrl) {
    ...
    return '' +
      ...
      '        <div style="font-weight:700; color:#0f172a; margin-top:22px; font-size:0.92rem;">Legislative Staff Secretariat</div>' +
      ...
      '        <div style="font-weight:700; color:#0f172a; margin-top:22px; font-size:0.92rem;">Legislative Officer</div>' +
  ...
  var _recentStaffReportsSearchQuery = '';
  var _recentStaffReportsCurrentPage = 1;
  var _recentStaffReportsPageSize = 10;

  function onRecentStaffReportsSearchInput() {
    var input = document.getElementById('recentStaffReportsSearchInput');
    var clearBtn = document.getElementById('recentStaffReportsClearSearchBtn');
    _recentStaffReportsSearchQuery = input ? input.value.trim().toLowerCase() : '';
    if (clearBtn) {
      clearBtn.style.display = _recentStaffReportsSearchQuery.length > 0 ? 'inline-flex' : 'none';
    }
    _recentStaffReportsCurrentPage = 1;
    renderRecentGeneratedReportsTable();
  }
  window.onRecentStaffReportsSearchInput = onRecentStaffReportsSearchInput;

  function clearRecentStaffReportsSearch() {
    var input = document.getElementById('recentStaffReportsSearchInput');
    var clearBtn = document.getElementById('recentStaffReportsClearSearchBtn');
    if (input) input.value = '';
    if (clearBtn) clearBtn.style.display = 'none';
    _recentStaffReportsSearchQuery = '';
    _recentStaffReportsCurrentPage = 1;
    renderRecentGeneratedReportsTable();
  }
  window.clearRecentStaffReportsSearch = clearRecentStaffReportsSearch;
  ...
  function renderRecentGeneratedReportsTable() {
    ...
    if (_recentStaffReportsSearchQuery) {
      var q = _recentStaffReportsSearchQuery;
    ...
    if (!list || list.length === 0) {
      var msg = _recentStaffReportsSearchQuery ? ('No generated reports found matching "' + esc(_recentStaffReportsSearchQuery) + '".') : 'No matching reports found for this filter.';
