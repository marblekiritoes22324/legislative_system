<section id="policyResearchSection"
  class="content-section <?= $active_section !== 'policyResearchSection' ? 'd-none' : '' ?>">
  ...
  <div class="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
      <div>
        <h2 class="h4 fw-bold text-dark mb-1"><i class="bi bi-file-earmark-text text-primary me-2"></i>Policy
          Research</h2>
        <p class="text-muted mb-0">Manage legislative ordinances, resolutions, policy drafts, and generate AI
          document summaries.</p>
      </div>
      <div class="d-flex flex-wrap align-items-center gap-2">
        <a href="admin_dashboard.php?section=policyResearchSection&status="
          class="btn <?= empty($status_filter) ? 'btn-primary' : 'btn-outline-primary' ?> fw-semibold rounded-3 shadow-sm d-inline-flex align-items-center gap-1">
          <i class="bi bi-folder-fill"></i> All Policies
        </a>
        <a href="admin_dashboard.php?section=policyResearchSection&status=Archived"
          class="btn <?= ($status_filter === 'Archived') ? 'btn-secondary' : 'btn-outline-secondary' ?> fw-semibold rounded-3 shadow-sm d-inline-flex align-items-center gap-1">
          <i class="bi bi-archive-fill"></i> Archived Policies
        </a>
        <button class="btn btn-warning fw-semibold rounded-3 shadow-sm" data-bs-toggle="modal"
    ...
    <style>
      ...
      /* Option 1: Deep Manila Navy Table Header */
    ...
    <form method="GET" action="admin_dashboard.php" class="d-flex flex-wrap flex-md-nowrap align-items-center gap-2.5 mb-4" id="policyFilterForm">
      ...
      <div class="d-flex align-items-center justify-content-end gap-2 ms-auto text-end" style="flex-shrink: 0;">
        <?php if (!empty($search) || !empty($category_filter) || $has_active_date): ?>
          <a href="admin_dashboard.php?section=policyResearchSection<?= !empty($status_filter) ? '&status=' . urlencode($status_filter) : '' ?>" 
              ...
              <tr>
                ...
                <td>
                  <div class="action-btn-group d-flex align-items-center gap-1.5">
                    ...
                    <?php else: ?>
                      <button class="btn btn-policy-action btn-policy-action-view opacity-50" disabled
                    ...
                    <?php if (($policy['status'] ?? '') === 'Archived'): ?>
                      <form method="POST" action="admin_dashboard.php" class="d-inline"
                        ...
                        <input type="hidden" name="section" value="policyResearchSection">
                        <button type="submit" class="btn btn-policy-action btn-policy-action-restore"
                          title="Restore Policy Record">
                      ...
                      <form method="POST" action="admin_dashboard.php" class="d-inline"
                        ...
                        <button type="submit" class="btn btn-policy-action btn-policy-action-delete"
