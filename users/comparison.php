<?php
// users/comparison.php — Councilor/User Policy Comparison & Benchmarking Submodule
...
<section id="policyComparisonSection"
  ...
  <div class="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white">
    ...
    <!-- Mode 1: Compare Policies Selectors (Bootstrap Input Groups) -->
    <div class="row g-3 align-items-end mb-4" id="userPolicyCompareForm">
          ...
          <select id="userComparePolicyB" class="form-select border-start-0 rounded-end-3" style="font-size:0.9rem;">
            ...
            <?php else: ?>
              ...
              <?php if (!empty($u_external_policies)): ?>
                <optgroup label="🏙️ External LGU Benchmarks (Quezon City, Pasig, etc.)">
                  <?php foreach ($u_external_policies as $p): ?>
                    <option value="<?= (int) $p['id'] ?>" <?= ($p === reset($u_external_policies)) ? 'selected' : '' ?>>
                      [<?= htmlspecialchars($p['city_origin'] ?? 'Benchmark') ?>] <?= htmlspecialchars($p['title']) ?>
                    </option>
                  <?php endforeach; ?>
                </optgroup>
  ...
  (function () {
    ...
    window.switchUserComparisonMode = function (mode) {
      ...
      if (mode === 'versions') {
        ...
        var verSelect = document.getElementById('userCompareVersionPolicy');
        if (verSelect && verSelect.value) {
          window.runUserVersionComparison();
        }
      } else {
        if (btnPolicies) {
    ...
    // --- DYNAMIC AI COMPARISON SYNTHESIS ENGINE ---
    ...
    window.runUserPolicyComparison = function () {
      var aId = document.getElementById('userComparePolicyA').value;
      var bId = document.getElementById('userComparePolicyB').value;
      var resultEl = document.getElementById('userComparisonResult');
      if (!resultEl) return;
      ...
      var find = function (id) {
        for (var i = 0; i < USER_COMPARE_DATA.length; i++)
          if (String(USER_COMPARE_DATA[i].id) === String(id)) return USER_COMPARE_DATA[i];
        return null;
      };
    ...
    // --- MODE 2: COMPARE VERSIONS (COUNCILORS) ---
    window.runUserVersionComparison = function () {
      ...
      var record = window.USER_VERSION_COMPARE_MAP[String(pId)];
      if (!record) {
